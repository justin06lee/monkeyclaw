...
The demo runs with **zero model credentials** — a pre-built knowledge base
feeds every dashboard view:

```bash
demo/run_hackathon_demo.sh            # seed the demo DB + open the dashboard
demo/run_hackathon_demo.sh live       # one real cycle, then the dashboard
```

Or drive it by hand:

```bash
uv run python demo/seed_demo_db.py    # (re)build a deterministic demo DB
...
- The eight-view live dashboard and the one-command fallback demo.
