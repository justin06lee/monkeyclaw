...
- **Demo seed** — `demo/seed_demo_db.py` plants findings drawn from the
  whitepaper's adversarial corpus (Appendix E) and detection catalog
  (Appendix D), so the demo reflects real, named failure classes.
