...
The fallback demo runs all of this from a seeded knowledge base with no
model credentials — so the demo never depends on a live API.
...
- **It is grounded.** The 18 zones map to recognized failure classes
  ...
  means something.
- **It demos without luck.** The seeded fallback path needs no network
  and no credentials.
